...
def split_and_prepare_data(data, option):
    ...
    if(option == '0' or option == '1'):
        ...
        eval_sub = []
        eval_sub.append(subjects[0])
        eval_sub.append(subjects[1])
        train_X = []
        train_Y = []
        test_X = []
        test_Y = []
        eval_X = []
        eval_Y = []
        train_sub = 'S15', 'S16', 'S17', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'S8', 'S9'
        test_sub = 'S13', 'S14'
        ...
        for sub in eval_sub:
            sub_data = data[data['subject'] == sub]
            eval_x = sub_data[feature_cols].values
            eval_y = sub_data[label_col].values
            eval_x_w, eval_y_w = create_windows(eval_x, eval_y, seq_len=SEQ_LEN)
            # train_x_w, test_x_w, scaler = normalize_data(train_x_w, test_x_w)
            eval_X.append(eval_x_w)
            eval_Y.append(eval_y_w)
        ...
        for sub in train_sub:
            ...
            train_Y_np = np.concatenate(train_Y)
            train_X_np = np.concatenate(train_X)
