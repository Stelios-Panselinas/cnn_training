...
from tensorflow.keras.layers import Conv1D, Conv2D, MaxPooling2D, GlobalAveragePooling2D, MaxPooling1D, Flatten, Dense, Dropout, GlobalAveragePooling1D, Input, BatchNormalization
...
def build_cnn(seq_len, num_features, num_classes=NUM_CLASSES):
    model = Sequential([
        Input(shape=(seq_len, num_features,1)),

        Conv2D(filters=FILTER1, kernel_size=(3,1), activation='relu', padding='same'),
        # BatchNormalization(),
        MaxPooling2D(pool_size=(2,1)),

        Conv2D(filters=FILTER2, kernel_size=(3,1), activation='relu', padding='same'),
        # BatchNormalization(),
...
def train_model(train_df, test_df, option):
    ...
    if(option == '0'):
        ...
        train_X = train_X.astype(np.float32).reshape(-1, SEQ_LEN, 8, 1)
        test_X  = test_X.astype(np.float32).reshape(-1, SEQ_LEN, 8, 1)
...
def create_qat_model(model, train_X, train_y, val_X, val_y):
    # 2. Create QAT model
    ...
    qat_model.summary()
    # 4. Fine-tune
    qat_model.fit(train_X, train_y, validation_data=(val_X, val_y), epochs=5)

    # 5. Convert to TFLite
    converter = tf.lite.TFLiteConverter.from_keras_model(qat_model)
    tflite_model = converter.convert()

    with open("cnn_qat.tflite", "wb") as f:
        f.write(tflite_model)
...
class Conv1DQuantizeConfig(tfmot.quantization.keras.QuantizeConfig):
    ...
    def get_activations_and_quantizers(self, layer):
        # Quantize the activation function used inside the layer.
        return [
            (
                layer.activation,
    ...
    def get_output_quantizers(self, layer):
        # Usually leave this empty for Conv/Dense in this pattern.
        return []
...
# -----------------------------
# Optional: custom config for Dense
# -----------------------------
...
def build_float_cnn(seq_len, num_features, num_classes=NUM_CLASSES):
    model = Sequential([
        ...
        MaxPooling1D(pool_size=POOL_SIZE, name='pool1'),
...
# -----------------------------
# Annotated model for manual QAT
# -----------------------------
def build_annotated_cnn(seq_len, num_features, num_classes=NUM_CLASSES):
    qa = tfmot.quantization.keras.quantize_annotate_layer
    ...
    model = Sequential([
        ...
        qa(
            Dense(num_classes, activation='softmax', name='classifier'),
            quantize_config=DenseQuantizeConfig(),
        )
    ])
    return model
...
# -----------------------------
# Build QAT model from pretrained float weights
# -----------------------------
def create_qat_model_from_float(float_model, seq_len, num_features, lr=1e-4):
    annotated_model = build_annotated_cnn(seq_len, num_features)

    # Build annotated model once so weights exist
    annotated_model(np.zeros((1, seq_len, num_features), dtype=np.float32))

    # Copy float weights layer by layer where names match
    float_layers = {layer.name: layer for layer in float_model.layers}
    for layer in annotated_model.layers:
        # QuantizeAnnotate wrappers still expose the inner layer config at apply-time,
        # but before apply they behave like normal layers for weights if names match.
        if layer.name in float_layers:
            try:
                layer.set_weights(float_layers[layer.name].get_weights())
            except Exception:
                pass
...
# Example usage
# -----------------------------
def manual_qat_pipeline(train_X, train_Y, test_X, test_Y):
    ...
    float_model = build_float_cnn(seq_len, num_features)
    float_model.compile(
        optimizer=Adam(learning_rate=1e-3),
        loss='sparse_categorical_crossentropy',
        metrics=['accuracy']
    ...
    # 2) Create QAT model from float model
    qat_model = create_qat_model_from_float(
...
if __name__ == "__main__":
    ...
    train_Y = np.stack(train_df["y"].values).astype(np.int32)
    train_X = np.stack(train_df['X'].values).astype(np.float32)
    test_X  = np.stack(test_df['X'].values).astype(np.float32)
    test_Y  = test_df['y'].values.astype(np.int32)
    float_model, qat_model = manual_qat_pipeline(
    train_X,
    train_Y,
    test_X,
    test_Y
    ...
    print("QAT TFLite model saved.")
    #     # --- IMPORTANT: reshape for Conv2D ---
    # train_X = train_X.astype(np.float32).reshape(-1, SEQ_LEN, 8, 1)
    # test_X  = test_X.astype(np.float32).reshape(-1, SEQ_LEN, 8, 1)

    # train_Y = train_Y.astype(np.int32)
    # test_Y  = test_Y.astype(np.int32)
    # create_qat_model(model, train_X, train_Y, test_X, test_Y)
